...
for crawl_id in progressbar(range(start_crawl_id, end_crawl_id + 1)):    
    ...
    cursor = ds.postgres.connection.cursor()
    cursor.execute('INSERT INTO crawl_analysis (crawl_id, mongo_analysis_id, loc_gov_id) VALUES (%s, %s) RETURNING id;', (crawl_id, str(result.inserted_id), analysis_doc['loc_gov_id']))
